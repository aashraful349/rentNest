...
- 🌐 **Live API Base URL**: [Live Link](https://rent-nest-ivory.vercel.app/)
- 📚 **Interactive Postman Documentation**: [Postman Docs](https://documenter.getpostman.com/view/46711953/2sBYAsyCiZ)
- 💻 **GitHub Repository**: [rentNest Github Repo](https://github.com/aashraful349/rentNest)
...
│   │   ├── 20260822131330
│   │   │   └── migration.sql
│   │   ├── 20260826030929_added_avavility_in_property
│   │   │   └── migration.sql
│   │   └── migration_lock.toml
│   ├── schema
│   │   ├── categories.prisma
...
- Landlord property creation (with default `AVAILABLE` status), editing, and deletion with ownership checks
- Tenant rental-request workflow (blocks requests on `UNAVAILABLE` properties) and landlord approval/rejection workflow
- Stripe Checkout payment-session creation and Stripe webhook processing (automatically updates property status to `UNAVAILABLE` upon completed payment)
...
For the property-list endpoint, each item contains summary data only: `id`, `availability`, `pName`, `pLocation`, `pPrice`, `createdAt`, and the category's `id` and `type`. It intentionally does not return the landlord ID, property description, image, or category description. Use `GET /api/properties/:id` to retrieve the full property record.
...
Creates a property owned by the logged-in landlord and creates its category at the same time. Landlords do not provide `availability` in the request payload; property availability defaults to `AVAILABLE` automatically upon creation.
...
Creates a rental request for the authenticated tenant. New requests begin with the status `PENDING`. Rental requests can only be made for properties with `availability: AVAILABLE`.
...
**Common errors:** `400` property unavailable for rent (`Property is not available for rent`) or invalid property foreign key, `401` missing token, `404` property not found.
...
It also automatically updates the rented property's `availability` to `UNAVAILABLE`.
...
2. Landlord creates a property (default availability: AVAILABLE).
3. Tenant searches properties and sends a rental request (only allowed if AVAILABLE).
...
7. The webhook records the completed payment and updates property availability to UNAVAILABLE.
...
| `Property`      | A rental listing owned by a landlord with availability status (`AVAILABLE` / `UNAVAILABLE`). | Has one optional category; has many rental requests and reviews. |
